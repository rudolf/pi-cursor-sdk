...
- After each finished SDK run, the provider calls `agent.getUsage()` (no `runId` on local agents; `runId` only for cloud `run-*` IDs) and maps billed spend into pi `usage` spend fields. Occupancy `totalTokens` uses only in-window local turn-ended occupancy below the session-scoped compact `tokensBefore` watermark recorded on `session_compact` (Pi's `convertToLlm()` does not pass `compactionSummary` / `tokensBefore` through to the provider); billed rows never become occupancy, so footer/auto-compact stay aligned with the post-compact Cursor prompt. The watermark is retired after the first accepted post-compact local occupancy so later legitimate growth can exceed the old `tokensBefore`. A matching pre-compact local resume handle is dropped on `session_compact`; the next acquire force-creates until the first post-compact `Agent.create` commits.
