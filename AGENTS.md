...
- `src/cursor-session-compaction-watermark.ts` owns the session-scoped post-`/compact` `tokensBefore` occupancy watermark and force-create-until-first-`Agent.create` flag; usage accounting reads the watermark instead of `role === "compactionSummary"` on converted LLM messages, then retires it after the first accepted post-compact local occupancy.
...
- `src/cursor-usage-accounting.ts` owns pi usage mapping from local turn-ended and billed `Agent.getUsage()` spend, plus post-compaction occupancy floors from the session compact watermark.
